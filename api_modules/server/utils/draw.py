...
def draw_stock(stock_id, stock_info, buy_day, sell_date=None, left_offset=5, right_offset=5):
    ...
    idx = (stock_info['date'] == buy_day) & (stock_info['code'] == stock_id)
    ...
    tmp_df = stock_info[idx].sort_values('date').reset_index()
    x = list(range(len(tmp_df)))
    tmp_df['index'] = x
    idx = (tmp_df['date'] == buy_day)
    x_loc = tmp_df[idx]['index'].values[0]
    y_loc = tmp_df[idx]['close'].values[0]
    # print(x_loc, y_loc)
    data = tmp_df[['index', 'open', 'close', 'high', 'low']].values
    mpf.candlestick_ochl(ax, data, width=0.1, colorup='r', colordown='g')
