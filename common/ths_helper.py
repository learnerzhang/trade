...
class Config(object):
	...
	def __loadConfiguration(self):
		configPath = os.path.join(BASE_DIR, 'ths_cfg.xml')
...
class Tlog(Msg):
	...
	def __str__(self):
		strvar = str(self.__action) + " " + str(self.__assetsName) + " " + str(self.__assetsCode) + " " + str(
			self.__price) + " " + str(self.__amount) + " " + str(self.__status) + " " + str(self.__comments)
		strvar = re.sub(' +', ' ', strvar)
		return strvar

	def __subject(self):
		strvar = "! " + time.strftime("%H:%M:%S %d-%m", time.localtime()) + " " + str(self.__action) + " " + str(
			self.__status)
		strvar = re.sub(' +', ' ', strvar)
		return strvar
...
class Logging(logging.Logger):
	...
	def __setFileHandler(self):
		path = os.path.join(BASE_DIR, 'logs/main/', self.__logType)
...
if __name__ == "__main__":
	...
	# --- Tlog
	tlog = Tlog(action='entrust buy', assetsName='stock', assetsCode='000000', price='45.93', amount='1000',
				status='successed', comments='')
	mail = Mail(tlog)
	print(tlog)
