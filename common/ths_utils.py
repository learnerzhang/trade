...
from common import ascmds
from common import ths_helper
...
class Service(object):
	...
	def __init__(self):
		self._config = ths_helper.Config()
		self.__logging = ths_helper.Logging(logType='service')
		self.__lock = Lock()
...
class Evolving(Base):
	...
	def __init__(self):
		...
		self.__logging = ths_helper.Logging(logType='env_prod')
	...
	def transfer(self, transferType="bank2broker", amount=100000):
		...
		def mailMe(action='', assetsName='', assetsCode='', price='', amount='', status='', comments=''):
			try:
				if self.__keepInformed:
					tlog = helper.Tlog(action=action, assetsName=assetsName, assetsCode=assetsCode, price=price,
									   amount=amount, status=status, comments=comments)
					helper.Mail(tlog)
			except Exception as e:
		...
		try:
			cmd = ascmds.astransfer + ' ' + transferType + ' ' + str(
				amount) + ' ' + self._config.bank_password + ' ' + self._config.broker_password
			res = os.popen(cmd).read().strip()
			ls = [x.strip() for x in res.split(',')]
			...
			if flag == "successed":
				...
				mailMe(action="transfer", assetsName=transferType, assetsCode='', price='', amount=amount,
					   status="successed")
			else:
				info = ' '.join(ls)
	...
	def issuingEntrust(self, stockCode, amount, price=None, tradingAction='buy', assetType=None):
		...
		def mailMe(action='', assetsName='', assetsCode='', price='', amount='', status='', comments=''):
			try:
				if self.__keepInformed:
					...
					tlog = helper.Tlog(action=action, assetsName=assetsName, assetsCode=assetsCode, price=priceLogged,
		...
		try:
			...
			cmd = ascmds.asissuingEntrust + ' ' + tradingAction + ' ' + assetType + ' ' + str(stockCode) + ' ' + str(
				price) + ' ' + str(amount)
			res = os.popen(cmd).read().strip()
			ls = [x.strip() for x in res.split(',')]
			...
			if flag == "successed":
				...
				contractNo = ls.pop(0)
				self.__logging.info(
					"issuing entrust successed: " + tradingAction + ' ' + assetType + ' ' + str(stockCode) + ' ' + str(
						price) + ' ' + str(amount))
				mailMe(action=tradingAction, assetsName=assetType, assetsCode=stockCode, price=price, amount=amount,
					   status="successed")
			else:
				info = ' '.join(ls)
				self.__logging.error(
					"issuing entrust failed: " + tradingAction + ' ' + assetType + ' ' + str(stockCode) + ' ' + str(
						price) + ' ' + str(amount) + '. Err: ' + info)
				mailMe(action=tradingAction, assetsName=assetType, assetsCode=stockCode, price=price, amount=amount,
					   status="failed")
		except Exception as e:
			self.__logging.error(
				"issuing entrust failed: " + tradingAction + ' ' + assetType + ' ' + str(stockCode) + ' ' + str(
					price) + ' ' + str(amount) + '. Err: ' + str(e))
			mailMe(action=tradingAction, assetsName=assetType, assetsCode=stockCode, price=price, amount=amount,
				   status="failed")
		self.__lock.unlock()
		return status, contractNo
	...
	def revokeEntrust(self, revokeType="allBuyAndSell", assetType="stock", contractNo=None):
		...
		try:
			...
			if flag == "successed":
				status = True
				self.__logging.info("revoke entrust successed: " + revokeType + " " + assetType + " " + str(
					contractNo) + ". Info: " + info)
				mailMe(action="revoke entrust " + revokeType, assetsName=assetType, assetsCode=contractNo, price='',
					   amount='', status="successed")
			else:
				self.__logging.error(
					"revoke entrust failed: " + revokeType + " " + assetType + " " + str(contractNo) + ". Err: " + info)
				mailMe(action="revoke entrust " + revokeType, assetsName=assetType, assetsCode=contractNo, price='',
					   amount='', status="failed")
		except Exception as e:
			self.__logging.error(
				"revoke entrust failed: " + revokeType + " " + assetType + " " + str(contractNo) + ". Err: " + str(e))
			mailMe(action="revoke entrust " + revokeType, assetsName=assetType, assetsCode=contractNo, price='',
				   amount='', status="failed")
		self.__lock.unlock()
		return status
		...
		try:
			...
			if result.get("status"):
				if queryType == "entrust":
					...
					self.__logging.error(
						"get IPO failed: " + queryType + " " + dateRange + ". Err: " + result.get('info'))
	...
	def liquidating(self):
		...
		try:
			...
			for _ in range(3):
				...
				if allHoldingShares.get("stock").get("status") and allHoldingShares.get("sciTech").get(
						"status") and allHoldingShares.get("gem").get("status"):
					l = []
					for assetType in ["stock", "sciTech", "gem"]:
						dt = allHoldingShares.get(assetType).get("data")
						if dt[0][0]:
							l.extend([[assetType, x[0], x[7]] for x in dt if
									  int(x[7])])  # a list of [资产类型, 股票代码, 可用数量], 过滤掉可用为 0 的资产
					# if l is not [] or [[]]
					if l and l[0]:
	...
	def entrustPortfolio(self, StockCodeAmountPriceList=[]):
		...
		for stockCode, amount, price in StockCodeAmountPriceList:
			try:
				status, contractNo = self.issuingEntrust(stockCode=stockCode, amount=amount, price=price,
														 tradingAction="buy")
...
class EvolvingSim():
	...
	def issuingEntrust(self, stockCode, amount, price=None, tradingAction='buy'):
		...
		try:
			...
			cmd = ascmds.asissuingEntrustSim + ' ' + tradingAction + ' ' + assetType + ' ' + str(stockCode) + ' ' + str(
				price) + ' ' + str(amount)
			res = os.popen(cmd).read().strip()
			ls = [x.strip() for x in res.split(',')]
			...
			if flag == "successed":
				...
				self.__logging.info(
					"issuing entrust successed: " + tradingAction + ' ' + assetType + ' ' + str(stockCode) + ' ' + str(
						price) + ' ' + str(amount) + ' contractNo ' + contractNo)
			else:
				info = ' '.join(ls)
				self.__logging.error(
					"issuing entrust failed: " + tradingAction + ' ' + assetType + ' ' + str(stockCode) + ' ' + str(
				...
				if allSimHoldingShares.get("status"):
					...
					if l and l[0]:
						for stockCode, availableQuantity in l:
							status, contractNo = self.issuingEntrust(stockCode=stockCode, amount=availableQuantity,
																	 tradingAction='sell')
						# 等待成交
						time.sleep(8)
					else:
